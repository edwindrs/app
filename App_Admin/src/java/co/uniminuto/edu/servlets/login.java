...
import co.uniminuto.edu.ws.Usuario;
...
public class login extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (estaRegistrado(idusuario, password)) {
                HttpSession sesion = request.getSession();
                sesion.setAttribute("login", idusuario);
    ...
    private boolean estaRegistrado(int idusuario, String password) {
        Usuario u = consultaUsuario(idusuario);
        return u.getPassword().equals(password) && u.getRol() == 0;
    }

    private static Usuario consultaUsuario(int idusuario) {
